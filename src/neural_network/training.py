...
class Trainer(abc.ABC):
    ...
    def fit(self, dl_train: DataLoader, dl_test: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            loss = sum(train_result.losses) / len(train_result.losses)
            train_loss.append(loss)
            train_acc.append(train_result.accuracy)
            test_result = self.test_epoch(dl_test, verbose=verbose, **kw)
            loss = sum(test_result.losses) / len(test_result.losses)
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        x, y = batch
        x = x.to(self.device, dtype=torch.float).unsqueeze(0)
        y = y.to(self.device, dtype=torch.float).unsqueeze(0)
        ...
        y_pred, self.state = self.model(x, self.state)
        ...
        loss.backward()
        self.optimizer.step()
        num_correct = torch.sum(y_pred.round() == y)
        # ========================
